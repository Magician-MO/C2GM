...
def calculate_activation_statistics(files, model, batch_size=1,
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the inception model.
    """
    act = get_activations(files, model, batch_size, dims, device, verbose)  # n*2048:这里需要debug跑一边看属性
...
def calculate_fid_given_paths(real_path, fake_path, batch_size, device, dims):
...
def fid_score(real_path, fake_path, device="CPU", batch_size=50):
    ...
    # parser.add_argument('--device', default='CPU', type=str, help='GPU to use (or CPU)')
    ...
    fid_value = calculate_fid_given_paths(real_path, fake_path, batch_size, device, dims)
    return fid_value
