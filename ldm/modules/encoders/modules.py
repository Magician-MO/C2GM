...
class FrozenOpenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self, arch="ViT-H-14", version="./data/checkpoints/open_clip_pytorch_model.bin", max_length=77, freeze=True, layer="last"):
        super().__init__()
        assert layer in self.LAYERS
...
class FrozenOpenCLIPImageEmbedder(AbstractEncoder):
    ...
    #TODO: 添加RS-CLIP的模型，使用CLIP编码遥感图像并提取特征（多层）
    def __init__(self, arch="ViT-H-14", version="./data/checkpoints/open_clip_pytorch_model.bin", max_length=77, freeze=True, layer="last"):
        super().__init__()
        assert layer in self.LAYERS
    ...
    def forward(self, images):
        z = self.encode_with_transformer(images).to(next(self.model.parameters()).device)
        return z

    def encode_with_transformer(self, images):
        images = self.preprocess(images).unsqueeze(0).to(next(self.model.parameters()).device)
