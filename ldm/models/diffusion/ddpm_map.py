...
class DDPM(pl.LightningModule, ImageLoggerMixin):
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,
        ...
        if self.parameterization == "eps":
            loss_vlb_weights = self.betas ** 2 / (2 * self.posterior_variance * to_torch(alphas) * (1 - self.alphas_cumprod))
        elif self.parameterization == "x0":
            loss_vlb_weights = 0.5 * np.sqrt(torch.Tensor(alphas_cumprod)) / (2. * 1 - torch.Tensor(alphas_cumprod))
        elif self.parameterization == "v":
            loss_vlb_weights = torch.ones_like(self.betas ** 2 / (2 * self.posterior_variance * to_torch(alphas) * (1 - self.alphas_cumprod)))
        else:
            loss_vlb_weights = 0.5 * np.sqrt(torch.Tensor(alphas_cumprod)) / (2. * 1 - torch.Tensor(alphas_cumprod))
        loss_vlb_weights[0] = loss_vlb_weights[1]
        self.register_buffer('loss_vlb_weights', loss_vlb_weights, persistent=False)
        assert not torch.isnan(self.loss_vlb_weights).all()
    ...
    def get_loss(self, pred, target, mean=True):
        #TODO: 添加不同的损失函数 (特征损失、CLIPScore损失)
    ...
    def p_losses(self, x_start, t, noise=None):
        ...
        loss_dict.update({f'{log_prefix}/simple': loss.mean()})
        loss_simple = loss.mean() * self.loss_simple_weight

        loss_vlb = (self.loss_vlb_weights[t] * loss).mean()
        loss_dict.update({f'{log_prefix}/vlb': loss_vlb})
...
class LatentDiffusion(DDPM):
    ...
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx):
        ...
        if self.scale_by_std and self.current_epoch == 0 and self.global_step == 0 and batch_idx == 0 and not self.restarted_from_ckpt:
            ...
            x = super().get_input(batch, self.target_key)
            x = x.to(self.device)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if self.parameterization == "x0":
            ...
            model_output = self.decode_first_stage(sampler._predict_xstart_from_eps(x_noisy, t, model_output))
            target = cond[self.target_key][0]
        else:
            raise NotImplementedError()
        ...
        loss_vlb = (self.loss_vlb_weights[t] * loss_vlb).mean()
    ...
    def progressive_denoising(self, cond, shape, verbose=True, callback=None, quantize_denoised=False,
        ...
        if cond is not None:
            if isinstance(cond, dict):
                cond = {key: cond[key][:batch_size]
                        if not isinstance(cond[key], list) else
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=50, ddim_eta=0., return_keys=None,
        ...
        log = dict()
        z, c, x, xrec, xc = self.get_input(batch, self.target_key,
