...
from ldm.modules.diffusionmodules.util import linear, timestep_embedding, zero_module

from .refmap_cond_encoder import MapDualSpadeRefEncoder, MapEncoder, MapRefEncoder
...
class RefMapAdapter(nn.Module):
    def __init__(
        ...
        sk=True,
        use_conv=True,
        merge_type="dual_spade",
    ):
        super(RefMapAdapter, self).__init__()
        self.channels = channels
        self.nums_rb = nums_rb
        self.merge_type = merge_type
        if self.merge_type == "ref_dual_spade":
            self.merge_encoder = MapDualSpadeRefEncoder(out_channel=cin*2) # cascade ref w/ spade
        elif self.merge_type == "ref":
            self.merge_encoder = MapRefEncoder(out_channel=cin*2) # cascade ref w/o spade
        elif self.merge_type == "origin":
            self.merge_encoder = MapEncoder(out_channel=cin*2) # w/o cascade ref
        else:
            raise NotImplementedError(f"merge_type {merge_type} not implemented")
    ...
    def forward(self, source, ref_scale):
        # unshuffle
        if self.merge_type == "ref_dual_spade" or self.merge_type == "ref":
            x = self.merge_encoder(source, ref_scale) # w/ cascade ref
        elif self.merge_type == "origin":
            x = self.merge_encoder(source) # w/o cascade ref
        else:
            raise NotImplementedError(f"merge_type {self.merge_type} not implemented")
        # extract features
        features = []
        x = self.conv_in(x)
        features.append(x)
